<?php
    ...
    class DAOShop{
        function select_cars(){  
            $sql = "SELECT c.cod_car, YEAR(c.enrollment_date) AS 'year', c.km, c.publication_date, c.color, c.price, c.power, c.doors , i.url_image AS 'image', m.description AS 'model', b.description AS 'brand', s.description AS 'state', pr.description AS 'province', ty.description AS 'fuel'
            FROM car c, image i, model m, brand b, state s, population p, province pr, type_motor ty
            WHERE c.chassis_number = i.chassis_number AND m.cod_model = c.cod_model AND b.cod_brand = m.cod_brand AND c.zip_code = p.zip_code AND p.cod_province = pr.cod_province AND c.cod_typemotor = ty.cod_fuel AND s.cod_state = c.cod_state AND i.url_image LIKE '%/prtd-%' ;";
        ...
        function select_one_car($id){  
            ...
            e.description AS 'environmental_label', t.description AS 'type_motor'
            FROM car c, model m, brand b, state s, population p, province pr, environmental_label e, type_motor t, cylinder_capacity cyl, bodywork bd, shifter sh
            ...
            AND c.cod_typemotor = t.cod_fuel AND c.cod_cylinder AND c.cod_cylinder = cyl.cod_cylinder AND c.cod_bodywork = bd.cod_bodywork AND c.cod_shifter = sh.cod_shifter
            AND c.cod_car = $id;";
    ...
    // MENU LATERAL
        ...
        function filters($filter){
            ...
            FROM (SELECT c.cod_car, YEAR(c.enrollment_date) AS 'year', c.km, c.publication_date, c.color, c.price, c.power, c.doors , i.url_image AS 'image',
                        ...
                        env.description AS 'environmental_label'
                        FROM car c, image i, model m, brand b, state s, population p, province pr, type_motor ty, shifter sh, environmental_label env
                        ...
                        AND c.cod_typemotor = ty.cod_fuel AND s.cod_state = c.cod_state AND sh.cod_shifter = c.cod_shifter AND env.cod_label = c.cod_label AND i.url_image LIKE '%/prtd-%') as kk";
